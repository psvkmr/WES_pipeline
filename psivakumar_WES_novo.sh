...
#${BWA} mem -t 24 -T 0 -M -R "@RG\tID:1\tPU:Unit\tLB:library\tSM:${NAME}\tPL:illumina" ${REF} ${FQ1} ${FQ2} > ${NAME}.aln.sam #add log?
#${BWA} mem -t 24 -T 0 -M -H -a -P -R "@RG\tID:1\tPU:Unit\tLB:library\tSM:${NAME}\tPL:illumina" ${REF} ${FQ1} ${FQ2} > ${NAME}.aln.sam #add log?
#${GATK} SortSam -I ${NAME}.aln.sam -SO coordinate -O ${OUT_BAM1}.withDups
#${GATK} MarkDuplicates -I ${OUT_BAM1}.withDups -M ${NAME}.markedDups.txt -O ${OUT_BAM1}
#${GATK} BuildBamIndex -I ${OUT_BAM1}
#${SAMTOOLS} view -Shb ${NAME}.aln.sam -o ${OUT_BAM1}.unsorted
#${SAMTOOLS} sort ${OUT_BAM1}.unsorted -o ${OUT_BAM1}
#${SAMTOOLS} index ${OUT_BAM1}

# GATK

#${GATK} RealignerTargetCreator -R ${REF} --known ${INDELS} -I ${OUT_BAM1} -o ${NAME}.realign.intervals
#${GATK} IndelRealigner -R ${REF} --known ${INDELS} -targetIntervals ${NAME}.realign.intervals --noOriginalAlignmentTags -I ${OUT_BAM1} -nWayOut ${NAME}.map
#${GATK} BaseRecalibrator --java-options '-DGATK_STACKTRACE_ON_USER_EXCEPTION=true' -R ${REF} -I ${OUT_BAM1} --known-sites ${SNPS} --known-sites ${DBSNPS} --known-sites ${INDELS} -O ${NAME}.bqsr.grp
#${GATK} BaseRecalibrator -R ${REF} -I ${OUT_BAM1} --BQSR ${NAME}.bqsr.grp -O ${NAME}.bqsr.grp #--plots ${NAME}.afterRecal.pdf
#${GATK} AnalyzeCovariates --bqsr ${NAME}.bqsr.grp --plots ${NAME}.covariates.pdf
#${GATK} ApplyBQSR  -R ${REF} -I ${OUT_BAM1} --bqsr-recal-file ${NAME}.bqsr.grp -O ${OUT_BAM2}
#${GATK} PrintReads -R ${REF} -I ${OUT_BAM1} --BQSR ${NAME}.bqsr.grp -o ${OUT_BAM2}
#${GATK} BuildBamIndex -I ${OUT_BAM2}
#${SAMTOOLS} index ${OUT_BAM2}
#${VBI} --vcf ${SNPS} --bam ${OUT_BAM2} --out ${NAME}.vbi
#${GATK} BamIndexStats -I ${OUT_BAM2} > ${NAME}.indexstats.txt
#${GATK} CollectWgsMetrics -R ${REF} -I ${OUT_BAM2} --INTERVALS ${KIT} --INCLUDE_BQ_HISTOGRAM -O ${NAME}.WGSmetrics.txt

# add dbsnp file
#${GATK} HaplotypeCaller -R ${REF} --intervals ${KIT} -I ${OUT_BAM2} -ERC GVCF -O ${NAME}.g.vcf.gz
# {GATK} CombineGVCFs / GenomicsDBImport
#${GATK} GenotypeGVCFs -R ${REF} -V ${NAME}.g.vcf.gz --annotate-with-num-discovered-alleles --create-output-variant-index -O ${NAME}.vcf.gz
#${TABIX} -p vcf ${NAME}.vcf.gz
#${GATK} ValidateVariants -R ${REF} -V ${NAME}.vcf.gz
#${GATK} VariantAnnotator -R ${REF} -I ${OUT_BAM2} -V ${NAME}.g.vcf.gz -L ${NAME}.g.vcf.gz -A MQ0 -A SpanningDeletions -o ${NAME}.reannotated.g.vcf.gz
#${GATK} CNNScoreVariants -R ${REF} -V ${NAME}.g.vcf.gz -O ${NAME}.CNNanno.vcf.gz
#${GATK} FilterVariantTranches
# implement VQSR
#bash ${VEP} ${NAME}.vcf.gz
