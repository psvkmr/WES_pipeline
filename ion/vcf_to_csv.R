...
processInfo <- function(vcf, csq) {
  vcf2 <- vcf %>%
    ...
    mutate_at(c("gnomADg_AF_afr", "gnomADg_AF_ami", "gnomADg_AF_amr", "gnomADg_AF_asj", "gnomADg_AF_eas", "gnomADg_AF_fin", "gnomADg_AF_nfe", "gnomADg_AF_oth", "gnomADg_AF_sas"), as.numeric) %>%
    rowwise() %>%
    mutate(MAX_AF_g = max(gnomADg_AF_afr, gnomADg_AF_ami, gnomADg_AF_amr, gnomADg_AF_asj, gnomADg_AF_eas, gnomADg_AF_fin, gnomADg_AF_nfe, gnomADg_AF_oth, gnomADg_AF_sas)) %>%
    replace_na(list(MAX_AF = 0, MAX_AF_g = 0)) %>%
    mutate_at(vars(contains("WXS")), ~gsub(":", "|", .)) %>%
    mutate_at(vars(contains("WXS")), ~gsub("0/1", "HET", .)) %>%
    mutate_at(vars(contains("WXS")), ~gsub("1/1", "HOM", .))
  return(vcf2)
}
